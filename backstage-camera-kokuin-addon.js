/* Backstage for Camera 名入れ刻印アドオン（有料オプション、+¥1,100税込）
   既存の backstage-camera-simulator.js（カラーシミュレーター本体）と同じページに読み込まれる前提。
   カラー選択のグローバル状態（backstageCameraColors, engravingColor）をそのまま参照する。

   Backstage for Bagと同様、backstage_camera_color_order.svg は商品イメージと刻印プレビューが
   最初から1枚に統合されている（<g id="kokuin"> が right1（data-name="right"）の中に存在）ため、
   別ファイルを取得してShadow DOMに展開する拡大プレビューは作らず、backstage-camera-simulator.js
   が読み込んだメインSVG（#backstage-camera-svg-wrap 内）の #kokuin グループへ直接テキストを
...
(function () {
  ...
  // backstage_camera_color_order.svg内、サンプル文字「ABCDEFGHIJK」が元々あった位置の中心
  // （プレート円の下、レザー面上。プレート円 cx=317.73/cy=98.4/r=20.63 とは重ならない）
  const ANCHOR = { x: 317.69, y: 148.08 };
  const ANGLE_DEG = 0;
  ...
  window.BACKSTAGE_CAMERA_KOKUIN_STATE = { enabled: false, text: '', fontFamily: '', valid: false };
  ...
  function mainKokuinGroup() {
    return document.querySelector('#backstage-camera-svg-wrap svg #kokuin');
  }

  // カラーシミュレーター本体で選ばれた色が変わった際にも呼ばれる（backstage-simulator.jsから）
  function applyBackstageCameraKokuinColors() {
    drawKokuinText();
  }
  window.applyBackstageCameraKokuinColors = applyBackstageCameraKokuinColors;

  function buildFontSelect() {
    const select = document.getElementById('backstage-camera-kokuin-font-select');
    if (!select) return;
    select.innerHTML = '';
    FONTS.forEach(f => {
  ...
  function validateAndDraw() {
    const input = document.getElementById('backstage-camera-kokuin-text');
    const countEl = document.getElementById('backstage-camera-kokuin-char-count');
    const warnEl = document.getElementById('backstage-camera-kokuin-warn');
    if (!input) return;
    ...
    window.BACKSTAGE_CAMERA_KOKUIN_STATE = {
  ...
  function updateFontSwatch() {
    const swatch = document.getElementById('backstage-camera-kokuin-font-swatch');
    if (!swatch) return;
    const font = currentFont();
  ...
  function drawKokuinText() {
    ...
    const baseHex = (typeof backstageCameraColors !== 'undefined') ? backstageCameraColors.leather : null;
  ...
  async function onToggleChange() {
    const toggle = document.getElementById('backstage-camera-kokuin-toggle');
    const section = document.getElementById('backstage-camera-kokuin-section');
    ...
    validateAndDraw();
    if (typeof updateBackstageCameraPriceDisplay === 'function') updateBackstageCameraPriceDisplay();
  }
  ...
  window.BACKSTAGE_CAMERA_KOKUIN_PRICE_ADD = KOKUIN_PRICE_ADD;
