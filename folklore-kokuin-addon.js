/* Folklore 名入れ刻印アドオン（有料オプション、+¥1,100税込）
   既存の simulator.js（カラーシミュレーター本体）と同じページに読み込まれる前提。
   Folkloreは10〜30枚のウロコパーツを個別に色付けできる仕様のため、刻印は
   「先頭（前/ボディ上部側）から3番目のウロコ」の色に連動させる（708works確認済み。
   通常そこには708worksロゴが型押しされるが、名入れ刻印時は表に刻印・裏にロゴの想定）。 */
(function () {
  const FONTS = [
  ...
  // Folklore用の配置基準（folklore-kokuin-simulator.jsと同じ値）
  ...
  function makeTextEl(font) {
    ...
    // レーザー刻印は革の色に関わらず視認できるため、刻印箇所の革色（先頭から3番目のウロコ）に
    // 対して コントラストが出る色（本体シミュレーターのロゴ刻印と同じロジック）を使う
    const baseHex = (typeof partColors !== 'undefined' && typeof N !== 'undefined') ? partColors[N - 3] : null;
    const fillColor = (baseHex && typeof getLogoColor === 'function') ? getLogoColor(baseHex) : '#2a1710';
