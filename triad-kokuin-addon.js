/* Triad 名入れ刻印アドオン（有料オプション、+¥1,100税込）
   既存の triad-simulator.js（カラーシミュレーター本体）と同じページに読み込まれる前提。
   カラー選択のグローバル状態（triadColors, TRIAD_ZONES, engravingColor）をそのまま参照する。 */
(function () {
  const FONTS = [
  ...
  // Triad用の配置基準（triad-kokuin-simulator.jsと同じ値）
  const ANCHOR = { x: 270.86, y: 327.15 };
  const ANGLE_DEG = 32.44;
  const BASE_FONT_SIZE = 26;
  const MAX_TEXT_WIDTH = 190;

  // triad_kokuin_base.svg内のクラス→ゾーン対応（triad_color_order.svgとは番号がズレている）
  const KOKUIN_ZONE_CLASS = { leather1: 'st2', leather2: 'st4', leather3: 'st3', parts: 'st5', hardware: 'st6' };
  ...
  window.TRIAD_KOKUIN_STATE = { enabled: false, text: '', fontFamily: '', valid: false };
  ...
  async function loadBaseSvg() {
    const wrap = document.getElementById('triad-kokuin-svg-wrap');
    if (!wrap || svgLoaded) return;
    const res = await fetch('https://708works-lab.github.io/dev/triad_kokuin_base.svg');
    const svgText = await res.text();
    // Shadow DOMで隔離する。カラーシミュレーター本体のSVGも同じ st1〜st6 のクラス名を
  ...
  function applyTriadKokuinColors() {
    const svg = kokuinShadowRoot?.querySelector('svg');
    if (!svg || typeof triadColors === 'undefined') return;
  ...
  function buildFontSelect() {
    const select = document.getElementById('triad-kokuin-font-select');
    if (!select) return;
    select.innerHTML = '';
    FONTS.forEach(f => {
  ...
  function validateAndDraw() {
    const input = document.getElementById('triad-kokuin-text');
    const countEl = document.getElementById('triad-kokuin-char-count');
    const warnEl = document.getElementById('triad-kokuin-warn');
    if (!input) return;
  ...
  function updateFontSwatch() {
    const swatch = document.getElementById('triad-kokuin-font-swatch');
    if (!swatch) return;
    const font = currentFont();
  ...
  async function onToggleChange() {
    const toggle = document.getElementById('triad-kokuin-toggle');
    const section = document.getElementById('triad-kokuin-section');
    ...
    validateAndDraw();
    if (typeof updateTriadPriceDisplay === 'function') updateTriadPriceDisplay();
  }
  ...
  window.TRIAD_KOKUIN_PRICE_ADD = KOKUIN_PRICE_ADD;
