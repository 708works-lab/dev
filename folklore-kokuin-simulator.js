/* Folklore Kokuin (engrave) Simulator — Phase 2 prototype
   実際の刻印箇所SVG（folklore_kokuin_base.svg）上に、選択フォント・入力文字で
...
(function () {
  ...
  // ── サンプル刻印パス（ABCDEFGHIJK）から算出した配置基準（folklore_kokuin_simulator.svg 2026-08-16） ──
  // 中心アンカー：先頭文字と末尾文字のbbox中心の中点
  // 角度：先頭→末尾の中心を結んだ直線の傾き（真ん中のリーフ型パーツに沿って右上がり）
  const VIEWBOX = { w: 765.65, h: 509.29 };
  const ANCHOR = { x: 379.17, y: 245.55 };
  const ANGLE_DEG = -60.7;
  const BASE_FONT_SIZE = 20;   // リーフ型パーツはTriadのベルトより幅が狭いため控えめに設定
  ...
  async function loadBaseSvg() {
    const res = await fetch('./folklore_kokuin_base.svg');
    const svgText = await res.text();
    document.getElementById('fk-svg-wrap').innerHTML = svgText;
  }

  // フォントの正式名称はお客様には見せず、「フォントA」等の記号名＋系統で表示する。
  // 正式名称（family）は注文内容の記録用にFONTS配列側に残す。
  function buildFontSelect() {
    const select = document.getElementById('fk-font-select');
    select.innerHTML = '';
    FONTS.forEach(f => {
  ...
  function validateAndDraw() {
    const input = document.getElementById('fk-text');
    const countEl = document.getElementById('fk-char-count');
    const warnEl = document.getElementById('fk-warn');
    const saveBtn = document.getElementById('fk-save-btn');
  ...
  function updateFontSwatch() {
    const swatch = document.getElementById('fk-font-swatch');
    const font = currentFont();
  ...
  function drawKokuinText() {
    ...
    // 2行とも同じ回転軸（ANCHOR）を中心に回転させることで、ベルト角度に対して垂直に積む
  ...
  async function saveImage() {
    ...
    try {
      const liveSvg = document.querySelector('#fk-svg-wrap svg');
      ...
      canvas.toBlob(blob => {
        ...
        a.href = dlUrl;
        a.download = `folklore-kokuin-preview-${Date.now()}.png`;
      ...
      console.error('[folklore-kokuin] save failed', e);
  ...
  async function init() {
    ...
    document.getElementById('fk-save-btn').addEventListener('click', saveImage);
