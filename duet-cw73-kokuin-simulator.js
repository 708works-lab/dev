/* Duet CW73 Kokuin (engrave) Simulator — Phase 2 prototype
   実際の刻印箇所SVG（duet_cw73_kokuin_base.svg）上に、選択フォント・入力文字で
...
(function () {
  ...
  // ── サンプル刻印パス（ABCDEFGHIJK）から算出した配置基準（duet_kokuin_simulator.svg CW73部分・2026-08-18差し替え版） ──
  // 中心アンカー：先頭文字と末尾文字のbbox中心の中点
  // 角度：先頭→末尾の中心を結んだ直線の傾き（リーフ型パーツに沿って右上がり）
  const VIEWBOX = { w: 711.6, h: 469.86 };
  const ANCHOR = { x: 358.71, y: 213.89 };
  ...
  async function loadBaseSvg() {
    const res = await fetch('./duet_cw73_kokuin_base.svg');
    const svgText = await res.text();
    document.getElementById('dc-svg-wrap').innerHTML = svgText;
  }

  // フォントの正式名称はお客様には見せず、「フォントA」等の記号名＋系統で表示する。
  // 正式名称（family）は注文内容の記録用にFONTS配列側に残す。
  function buildFontSelect() {
    const select = document.getElementById('dc-font-select');
    select.innerHTML = '';
    FONTS.forEach(f => {
  ...
  function validateAndDraw() {
    const input = document.getElementById('dc-text');
    const countEl = document.getElementById('dc-char-count');
    const warnEl = document.getElementById('dc-warn');
    const saveBtn = document.getElementById('dc-save-btn');
  ...
  function updateFontSwatch() {
    const swatch = document.getElementById('dc-font-swatch');
    const font = currentFont();
  ...
  async function saveImage() {
    ...
    try {
      const liveSvg = document.querySelector('#dc-svg-wrap svg');
      ...
      canvas.toBlob(blob => {
        ...
        a.href = dlUrl;
        a.download = `duet-cw73-kokuin-preview-${Date.now()}.png`;
      ...
      console.error('[duet-cw73-kokuin] save failed', e);
  ...
  async function init() {
    ...
    document.getElementById('dc-save-btn').addEventListener('click', saveImage);
